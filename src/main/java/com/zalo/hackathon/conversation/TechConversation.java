...
public class TechConversation {
    ...
    public static void main(String args[]) throws Exception {
        ...
        conversation.processRawMessage("Tôi muốn tìm sạc Iphone 7");
    }
    ...
    public void processShowMore() throws APIException {
        ...
        currentShowProducts = cachedProducts.subList(currentIndexProduct, Math.min(currentIndexProduct + SHOW_MORE_BATCH, cachedProducts.size()));
        sendProduct(currentShowProducts);
        if (currentIndexProduct + SHOW_MORE_BATCH >= cachedProducts.size()) {
    ...
    public void askDetail(Map<EntityType, List<Entity>> entities, Set<Intent> intent) throws APIException {
        if (entities.getOrDefault(EntityType.ORDER, new ArrayList<>()).size() > 0) {
            ...
            StringBuilder replyMessage = new StringBuilder().append("Thông số của sản phẩm " + productInfo.getTitle() + " đây ạ: ").append("\n");
    ...
    public void findItem(Map<EntityType, List<Entity>> entities, Set<Intent> intent) throws APIException {
        ...
        if (mainType == EntityType.DIEN_THOAI) {
            boolQuery = boolQuery.must(QueryBuilders.matchQuery("category", DIEN_THOAI));
        } else if (mainType == EntityType.PHU_KIEN) {
            boolQuery = boolQuery.must(QueryBuilders.matchQuery("category", PHU_KIEN));
        }

        boolQuery = boolQuery.must(QueryBuilders.queryStringQuery(keyword.toString()));

        LogCenter.info(LOG, boolQuery.toString());
        SearchResponse response = elasticDao.query(boolQuery
                .must(QueryBuilders.queryStringQuery(keyword.toString())), Config.INDEX, 100);
    ...
    public BoolQueryBuilder exact(String name, String value) {
        ...
        for (String word : name.split(",")) {
            bool = bool.must(QueryBuilders.termQuery(name, word));
        }

        return bool.minimumShouldMatch(name.split(",").length);
    }
